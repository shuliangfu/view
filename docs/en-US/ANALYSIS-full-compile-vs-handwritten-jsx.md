...
| **vIf / vElseIf / vElse chain**                          | ✓; root `vIf` wrapped in **`insertReactive`**                                      | ✓ on **`Fragment`** siblings. **Root intrinsic `vIf={getter}`**: evaluated **once** per `mountVNodeTree` pass unless the whole VNode is rebuilt inside a reactive `insertReactive` getter — **not** the same as compiled root `vIf` |
| **vOnce / vCloak**                                       | ✓                                                                                  | ✓                                                                                                                                                                                                                                   |
...
| **Controlled `value` / `checked` as signal getter**      | **`createEffect`** writes `el.value` / `checked`                                   | **✗ no auto binding**: `applyIntrinsicVNodeProps` **skips** non-event **functions**; reactive `value`/`checked` need full compile, outer `insertReactive` rebuild, or non-VNode strategies                                          |
| **`{...spread}` on intrinsic**                           | `spreadIntrinsicProps`                                                             | **✗** — merge props yourself or call `spreadIntrinsicProps`                                                                                                                                                                         |
| **Component `{...props}`**                               | `mergeProps` / helpers                                                             | **✗** — merge yourself                                                                                                                                                                                                              |
| **`className` / `htmlFor`**                              | mapped to `class` / `for`                                                          | ✓                                                                                                                                                                                                                                   |
| **`style` object**                                       | compile path handles                                                               | VNode path: **object `style` skipped** in `applyIntrinsicVNodeProps`                                                                                                                                                                |
| **SVG / `createElementNS`**                              | ✓                                                                                  | ✓                                                                                                                                                                                                                                   |
...
- **Hand-written**: **`Fragment` sibling chains** match compile semantics.
  **Single intrinsic node with `vIf`** only re-evaluates when **`mountVNodeTree`
  runs again** (e.g. parent `insertReactive` getter returns a new tree).
