/**
 * @fileoverview SSR 指令真实测试：renderToString 在 vIf/vElse/vElseIf/vFor/vShow/vText/vHtml 下的实际 HTML 输出
 */
...
describe("SSR vText / vHtml", () => {
  ...
  it("vHtml 默认输出原始 HTML", () => {
    const html = renderToString(() => ({
      type: "div",
      props: { vHtml: "<em>hi</em>" },
      children: [],
    }));
    expect(html).toContain("<em>hi</em>");
  });

  it("allowRawHtml: false 时 vHtml 输出转义", () => {
    const html = renderToString(
      () => ({
        type: "div",
        props: { vHtml: "<em>hi</em>" },
