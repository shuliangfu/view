...
describe("getRoutePathForChangedPath", () => {
  it("路径含 /routes/home 或 /routes/index 应返回 \"/\"", () => {
    ...
    expect(getRoutePathForChangedPath("examples/src/routes/home/index.tsx")).toBe("/");
  ...
  it("路径含 /routes/{segment} 应返回 \"/{segment}\"", () => {
    expect(getRoutePathForChangedPath("src/routes/signal/index.tsx")).toBe("/signal");
    expect(getRoutePathForChangedPath("src/routes/about.tsx")).toBe("/about");
    expect(getRoutePathForChangedPath("src/routes/user/profile.tsx")).toBe("/user");
    expect(getRoutePathForChangedPath("/proj/routes/docs/readme.tsx")).toBe("/docs");
  });

  it("路径不含 /routes/ 应返回 undefined", () => {
    expect(getRoutePathForChangedPath("src/components/Button.tsx")).toBeUndefined();
