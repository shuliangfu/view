...
| `onCleanup`                                                 | effect 内注册清理函数                   | ✅        | Signal（定时器 + onCleanup 演示区）                                 |
| `batch`                                                     | 手动批处理多次 set                      | ✅        | Signal（batch 与 effect 计数对比）                                  |
...
## 四、指令与 JSX 编译（jsx-compiler）

| 说明                                                                                                                                                                                                      | 示例覆盖                     |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| 当前 **@dreamer/view 主入口**（`deno.json` exports）**未导出** 独立 `directive` 子路径；浏览器示例路由中 **无**「Directive」页。                                                                          | ❌ 无独立示例页              |
| 条件/多分支、v-once、v-cloak 等 **compileSource 指令语法** 由 **`@dreamer/view/compiler`**（或 jsx-compiler 文档）描述；CLI 构建默认 **runtime JSX**，示例站与主包演示一致走 **Show / Switch / For** 等。 | 控制流见 **Control Flow** 页 |

**结论**：多页面 examples **不覆盖** compile-time 指令
API；需要指令级演示时请本地使用 `jsx-compiler`/`compileSource` 或查阅包内
`tests/unit` 与文档。
...
| Directive / jsx-compiler                 | compileSource 指令系（主包未导出 directive 子路径）                                                                                        | examples 无独立页；见 jsx-compiler 单测与文档 |
