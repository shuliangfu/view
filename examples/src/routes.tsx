/**
 * 多页面示例 — 路由配置
 * 使用 @dreamer/view/router 实现 SPA 无刷新切换，history 模式，链接直接写 href="/path"。
 */

import type { RouteConfig } from "@dreamer/view/router";
import type { VNode } from "@dreamer/view";
import { SignalDemo } from "./SignalDemo.tsx";
import { StoreDemo } from "./StoreDemo.tsx";
import { BoundaryDemo } from "./BoundaryDemo.tsx";
import { DirectiveDemo } from "./DirectiveDemo.tsx";
import { ReactiveDemo } from "./ReactiveDemo.tsx";
import { ResourceDemo } from "./ResourceDemo.tsx";
import { ContextDemo } from "./ContextDemo.tsx";
import { RuntimeDemo } from "./RuntimeDemo.tsx";
import { RouterDemo } from "./RouterDemo.tsx";
...
/** 首页：Hero（渐变+层次）+ 功能模块卡片（图标+强阴影+按钮式 CTA） */
function HomePage(): VNode {
  return (
    <div className="space-y-12">
      {/* Hero：渐变背景、更大标题、副标题层次 */}
      <section className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white via-indigo-50/30 to-slate-50 p-8 shadow-xl backdrop-blur dark:border-slate-600/80 dark:from-slate-800 dark:via-indigo-950/20 dark:to-slate-800/90 sm:p-12">
      ...
      {/* 功能模块网格：图标 + 强阴影 + hover 上浮 + 按钮式「进入示例」 */}
...
export const routes: RouteConfig[] = [
  { path: "/", component: (_match) => <HomePage />, meta: { title: "首页" } },
  {
    path: "/signal",
    component: (_match) => <SignalDemo />,
    meta: { title: "Signal" },
  },
  {
    path: "/store",
    component: (_match) => <StoreDemo />,
    meta: { title: "Store" },
  },
  {
    path: "/boundary",
    component: (_match) => <BoundaryDemo />,
    meta: { title: "Boundary" },
  },
  {
    path: "/directive",
    component: (_match) => <DirectiveDemo />,
    meta: { title: "指令" },
  },
  {
    path: "/reactive",
    component: (_match) => <ReactiveDemo />,
    meta: { title: "Reactive" },
  },
  {
    path: "/resource",
    component: (_match) => <ResourceDemo />,
    meta: { title: "Resource" },
  },
  {
    path: "/context",
    component: (_match) => <ContextDemo />,
    meta: { title: "Context" },
  },
  {
    path: "/runtime",
    component: (_match) => <RuntimeDemo />,
    meta: { title: "Runtime" },
  },
  {
    path: "/router",
    component: (_match) => <RouterDemo />,
    meta: { title: "Router" },
  },
];
...
export const notFoundRoute: RouteConfig = {
  path: "*",
  component: (_match) => <NotFoundPage />,
  meta: { title: "404" },
};

/** 从路由表派生的导航项，供 Layout 自动生成链接（path + basePath 即为 href） */
